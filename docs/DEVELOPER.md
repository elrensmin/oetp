...
The ledger anchors Merkle roots to a blockchain (Polygon PoS in v1). Anchors happen at:
...
The `AnchorBackend` trait abstracts blockchain interaction. Implement it for any chain:
...
| `/v1/ledger/anchors` | POST   | No   | List all blockchain anchors                        |
...
| `/v1/ledger/anchors` | POST   | No   | List all blockchain anchors for a tenant/exam                         |

The verification portal (Phase 2) calls these endpoints and cross-references against blockchain RPCs.
